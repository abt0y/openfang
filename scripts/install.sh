#!/usr/bin/env bash
# Tapthe.ai installer — works on Linux, macOS, WSL
# Usage: curl -sSf https://tapthe-ai.sh | sh
#
# Environment variables:
#   TAPTHE_AI_INSTALL_DIR  — custom install directory (default: ~/.tapthe-ai/bin)
#   TAPTHE_AI_VERSION      — install a specific version tag (default: latest)

set -euo pipefail

REPO="RightNow-AI/tapthe-ai"
INSTALL_DIR="${TAPTHE_AI_INSTALL_DIR:-$HOME/.tapthe-ai/bin}"
...
detect_platform() {
    ...
    case "$OS" in
        ...
        mingw*|msys*|cygwin*)
            ...
            echo "    irm https://tapthe-ai.sh/install.ps1 | iex"
            echo ""
            echo "  Or download the .msi installer from:"
            echo "    https://github.com/$REPO/releases/latest"
            echo ""
            echo "  Or install via cargo:"
            echo "    cargo install --git https://github.com/$REPO tapthe-ai-cli"
            exit 1
            ;;
...
install() {
    ...
    echo ""
    echo "  Tapthe.ai Installer"
    echo "  =================="
    echo ""

    # Get latest version
    if [ -n "${TAPTHE_AI_VERSION:-}" ]; then
        VERSION="$TAPTHE_AI_VERSION"
    ...
    URL="https://github.com/$REPO/releases/download/$VERSION/tapthe-ai-$PLATFORM.tar.gz"
    CHECKSUM_URL="$URL.sha256"

    echo "  Installing Tapthe.ai $VERSION for $PLATFORM..."
    mkdir -p "$INSTALL_DIR"

    # Download to temp
    TMPDIR=$(mktemp -d)
    ARCHIVE="$TMPDIR/tapthe-ai.tar.gz"
    ...
    if [ "$OS" = "darwin" ]; then
        if command -v xattr &>/dev/null; then
            xattr -cr "$INSTALL_DIR/tapthe-ai" 2>/dev/null || true
        fi
        if command -v codesign &>/dev/null; then
            if ! codesign --force --sign - "$INSTALL_DIR/tapthe-ai"; then
                echo ""
                echo "  Warning: ad-hoc code signing failed."
                echo "  On Apple Silicon, the binary may be killed (SIGKILL) by Gatekeeper."
                echo "  Try manually: xattr -cr $INSTALL_DIR/tapthe-ai && codesign --force --sign - $INSTALL_DIR/tapthe-ai"
                echo ""
            fi
        fi
    fi
    ...
    if [ -n "$SHELL_RC" ] && ! grep -q "tapthe-ai" "$SHELL_RC" 2>/dev/null; then
    ...
    # Verify installation
    if "$INSTALL_DIR/tapthe-ai" --version >/dev/null 2>&1; then
        INSTALLED_VERSION=$("$INSTALL_DIR/tapthe-ai" --version 2>/dev/null || echo "$VERSION")
        echo ""
        echo "  Tapthe.ai installed successfully! ($INSTALLED_VERSION)"
    else
        echo ""
        echo "  Tapthe.ai binary installed to $INSTALL_DIR/tapthe-ai"
    fi
