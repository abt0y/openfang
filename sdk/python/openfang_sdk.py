"""
Tapthe.ai Python SDK — helper library for writing Python agents.

Usage:

    from tapthe_ai_sdk import Agent
...
Or for simple scripts without the decorator pattern:
    ...
    from tapthe_ai_sdk import read_input, respond
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the Tapthe.ai kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("TAPTHE_AI_AGENT_ID", "")
        message = os.environ.get("TAPTHE_AI_MESSAGE", "")
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the Tapthe.ai kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in Tapthe.ai daemon logs)."""
...
if __name__ == "__main__":
    print("Tapthe.ai Python SDK")
    print("====================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from tapthe_ai_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
