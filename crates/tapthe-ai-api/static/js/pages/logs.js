// Tapthe.ai Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    startStreaming: function() {
      ...
      var sep = '?';
      var token = TaptheAiAPI.getToken();
    ...
    async fetchLogs() {
      ...
      try {
        var data = await TaptheAiAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'tapthe-ai-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await TaptheAiAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          TaptheAiToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          TaptheAiToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        TaptheAiToast.error('Chain verification failed: ' + e.message);
      }
    },
