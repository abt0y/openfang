// Tapthe.ai Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async loadChannels() {
      ...
      this.loadError = '';
      try {
        var data = await TaptheAiAPI.get('/api/channels');
    ...
    async startQR() {
      ...
      try {
        var result = await TaptheAiAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          TaptheAiToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await TaptheAiAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            TaptheAiToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await TaptheAiAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await TaptheAiAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            TaptheAiToast.success(this.setupModal.display_name + ' activated!');
          } else {
            TaptheAiToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          TaptheAiToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
    ...
    async removeChannel() {
      ...
      var self = this;
      TaptheAiToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await TaptheAiAPI.delete('/api/channels/' + name + '/configure');
          TaptheAiToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await TaptheAiAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          TaptheAiToast.success(result.message);
        } else {
          TaptheAiToast.error(result.message);
        }
      } catch(e) {
        TaptheAiToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        TaptheAiToast.success('Copied to clipboard');
      } catch(e) {
        TaptheAiToast.error('Copy failed');
      }
    },
