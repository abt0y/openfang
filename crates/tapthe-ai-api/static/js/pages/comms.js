// Tapthe.ai Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          TaptheAiAPI.get('/api/comms/topology'),
          TaptheAiAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      var self = this;
      var url = TaptheAiAPI.baseUrl + '/api/comms/events/stream';
      if (TaptheAiAPI.apiKey) url += '?token=' + encodeURIComponent(TaptheAiAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await TaptheAiAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await TaptheAiAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        TaptheAiToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        TaptheAiToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await TaptheAiAPI.post('/api/comms/task', body);
        TaptheAiToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        TaptheAiToast.error(e.message || 'Task failed');
      }
