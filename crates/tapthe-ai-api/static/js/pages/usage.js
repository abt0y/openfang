// Tapthe.ai Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await TaptheAiAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await TaptheAiAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await TaptheAiAPI.get('/api/usage/daily');
