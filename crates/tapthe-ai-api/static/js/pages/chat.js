// Tapthe.ai Chat Page — Agent chat with markdown + streaming
'use strict';
  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            TaptheAiAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      TaptheAiAPI.get('/api/models').then(function(data) {
        ...
        TaptheAiToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      TaptheAiAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function(resp) {
        ...
        TaptheAiToast.success('Switched to ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        TaptheAiToast.error('Switch failed: ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      TaptheAiAPI.get('/api/commands').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            TaptheAiAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              TaptheAiToast.success('Session reset');
            }).catch(function(e) { TaptheAiToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            TaptheAiAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { TaptheAiToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            TaptheAiAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { TaptheAiToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && TaptheAiAPI.isWsConnected()) {
            TaptheAiAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
        ...
        case '/verbose':
          if (self.currentAgent && TaptheAiAPI.isWsConnected()) {
            TaptheAiAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected. Connect to an agent first.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && TaptheAiAPI.isWsConnected()) {
            TaptheAiAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
          ...
          break;
        case '/status':
          TaptheAiAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              TaptheAiAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { TaptheAiToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          TaptheAiAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          TaptheAiAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          TaptheAiAPI.get('/api/a2a/agents').then(function(res) {
    ...
    selectAgent(agent) {
      ...
      if (!localStorage.getItem('of-chat-tips-seen')) {
        ...
        this.messages.push({
          id: ++localMsgId,
          role: 'system',
          text: '**Welcome to Tapthe.ai Chat!**\n\n' +
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await TaptheAiAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await TaptheAiAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await TaptheAiAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof TaptheAiToast !== 'undefined') TaptheAiToast.success('New session created');
      } catch(e) {
        if (typeof TaptheAiToast !== 'undefined') TaptheAiToast.error('Failed to create session');
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await TaptheAiAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof TaptheAiToast !== 'undefined') TaptheAiToast.error('Failed to switch session');
      }
    },
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await TaptheAiAPI.upload(this.currentAgent.id, att.file);
            ...
            TaptheAiToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      if (TaptheAiAPI.wsSend(wsPayload)) {
      ...
      // HTTP fallback
      if (!TaptheAiAPI.isWsConnected()) {
        TaptheAiToast.info('Using HTTP mode (no streaming)');
      }
      ...
      try {
        ...
        var res = await TaptheAiAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      TaptheAiAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      TaptheAiToast.confirm('Stop Agent', 'Stop agent "' + name + '"? The agent will be shut down.', async function() {
        try {
          await TaptheAiAPI.del('/api/agents/' + self.currentAgent.id);
          TaptheAiAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          TaptheAiToast.success('Agent "' + name + '" stopped');
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          TaptheAiToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          TaptheAiToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof TaptheAiToast !== 'undefined') TaptheAiToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await TaptheAiAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof TaptheAiToast !== 'undefined') TaptheAiToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
