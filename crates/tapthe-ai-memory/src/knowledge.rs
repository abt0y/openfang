...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> TaptheAiResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> TaptheAiResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| TaptheAiError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> TaptheAiResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| TaptheAiError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| TaptheAiError::Memory(e.to_string()))?;
