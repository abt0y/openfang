...
//! to Tapthe.ai users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use tapthe_ai_types::agent::UserId;
use tapthe_ai_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Tapthe.ai user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Tapthe.ai UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> TaptheAiResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| TaptheAiError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(TaptheAiError::AuthDenied(format!(
