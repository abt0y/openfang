//! Minimal `.env` file loader/saver for `~/.tapthe-ai/.env`.
//!
...
fn dotenv_tapthe_ai_home() -> Option<PathBuf> {
    if let Ok(home) = std::env::var("TAPTHE_AI_HOME") {
        return Some(PathBuf::from(home));
    }
    dirs::home_dir().map(|h| h.join(".tapthe-ai"))
}

/// Return the path to `~/.tapthe-ai/.env`.
pub fn env_file_path() -> Option<PathBuf> {
    dotenv_tapthe_ai_home().map(|h| h.join(".env"))
}

/// Load `~/.tapthe-ai/.env` and `~/.tapthe-ai/secrets.env` into `std::env`.
///
...
/// Return the path to `~/.tapthe-ai/secrets.env`.
pub fn secrets_env_path() -> Option<PathBuf> {
    dotenv_tapthe_ai_home().map(|h| h.join("secrets.env"))
}
...
/// Remove a key from `~/.tapthe-ai/.env`.
///
...
/// List key names (without values) from `~/.tapthe-ai/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Tapthe.ai environment — managed by `tapthe-ai config set-key`\n");
