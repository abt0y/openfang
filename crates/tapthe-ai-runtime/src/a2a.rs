...
//! - `build_agent_card` — expose Tapthe.ai agents via A2A
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not Tapthe.ai skills).
...
/// A2A skill descriptor (not an Tapthe.ai skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[tapthe_ai_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose Tapthe.ai agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an Tapthe.ai agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "Tapthe.ai/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use tapthe_ai_types::config::{A2aConfig, ExternalAgent};
