...
use tapthe_ai_kernel::config::tapthe_ai_home;
...
/// `~/.tapthe-ai/agents/{name}/agent.toml`, then spawns the agent.
#[tauri::command]
pub fn import_agent_toml(
    ...
    let agent_dir = tapthe_ai_home().join("agents").join(&agent_name);
...
///
/// Copies the selected file to `~/.tapthe-ai/skills/` and triggers a
...
pub fn import_skill_file(
    ...
    let skills_dir = tapthe_ai_home().join("skills");
...
/// Open the Tapthe.ai config directory (`~/.tapthe-ai/`) in the OS file manager.
#[tauri::command]
pub fn open_config_dir() -> Result<(), String> {
    let dir = tapthe_ai_home();
    std::fs::create_dir_all(&dir).map_err(|e| format!("Failed to create config dir: {e}"))?;
    open::that(&dir).map_err(|e| format!("Failed to open directory: {e}"))
}

/// Open the Tapthe.ai logs directory (`~/.tapthe-ai/logs/`) in the OS file manager.
#[tauri::command]
pub fn open_logs_dir() -> Result<(), String> {
    let dir = tapthe_ai_home().join("logs");
