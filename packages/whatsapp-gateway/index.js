...
const TAPTHE_AI_URL = (process.env.TAPTHE_AI_URL || 'http://127.0.0.1:4200').replace(/\/+$/, '');
const DEFAULT_AGENT = process.env.TAPTHE_AI_DEFAULT_AGENT || 'assistant';
...
async function startConnection() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Tapthe.ai', 'Desktop', '1.0.0'],
  });
  ...
  // Incoming messages → forward to Tapthe.ai
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Forward to Tapthe.ai agent
      try {
        const response = await forwardToTapthe.ai(text, phone, pushName, metadata);
        if (response && sock) {
...
// ---------------------------------------------------------------------------
// Forward incoming message to Tapthe.ai API, return agent response
// ---------------------------------------------------------------------------
function forwardToTapthe.ai(text, phone, pushName, metadata) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(`${TAPTHE_AI_URL}/api/agents/${encodeURIComponent(DEFAULT_AGENT)}/message`);
    ...
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Tapthe.ai API timeout'));
    });
    req.write(payload);
    req.end();
  });
...
// ---------------------------------------------------------------------------
// Send a message via Baileys (called by Tapthe.ai for outgoing)
// ---------------------------------------------------------------------------
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`[gateway] Tapthe.ai URL: ${TAPTHE_AI_URL}`);
