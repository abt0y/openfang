# Migrating to Tapthe.ai

This guide covers migrating from OpenClaw (and other frameworks) to Tapthe.ai. The migration engine handles config conversion, agent import, memory transfer, channel re-configuration, and skill scanning.
...
This auto-detects your OpenClaw workspace at `~/.openclaw/` and imports everything into `~/.tapthe-ai/`.

### Options

```bash
# Specify a custom source directory
tapthe-ai migrate --from openclaw --source-dir /path/to/openclaw/workspace
...
After a successful migration, a `migration_report.md` file is saved to `~/.tapthe-ai/` with a summary of everything that was imported, skipped, or needs manual attention.
...
```bash
tapthe-ai migrate --from langchain   # Coming soon
tapthe-ai migrate --from autogpt     # Coming soon
```
...
| Item | Source (OpenClaw) | Destination (Tapthe.ai) | Status |
|------|-------------------|------------------------|--------|
| **Config** | `~/.openclaw/config.yaml` | `~/.tapthe-ai/config.toml` | Fully automated |
| **Agents** | `~/.openclaw/agents/*/agent.yaml` | `~/.tapthe-ai/agents/*/agent.toml` | Fully automated |
| **Memory** | `~/.openclaw/agents/*/MEMORY.md` | `~/.tapthe-ai/agents/*/imported_memory.md` | Fully automated |
| **Channels** | `~/.openclaw/messaging/*.yaml` | `~/.tapthe-ai/channels_import.toml` | Automated (manual merge) |
...
Channel configurations (Telegram, Discord, Slack) are exported to a `channels_import.toml` file. You must manually merge the `[channels]` section into your `~/.tapthe-ai/config.toml`.
...
```bash
tapthe-ai skill install <skill-name-or-path>
```

Tapthe.ai automatically detects OpenClaw-format skills and converts them during installation.
...
### 1. Initialize Tapthe.ai

```bash
tapthe-ai init
```

This creates `~/.tapthe-ai/` with a default `config.toml`.
...
**Tapthe.ai** (`~/.tapthe-ai/config.toml`):
```toml
[default_model]
provider = "anthropic"
model = "claude-sonnet-4-20250514"
...
**Tapthe.ai** (`~/.tapthe-ai/agents/coder/agent.toml`):
```toml
name = "coder"
version = "0.1.0"
description = "A coding assistant"
...
**Tapthe.ai** (add to `~/.tapthe-ai/config.toml`):
```toml
[channels.telegram]
bot_token_env = "TELEGRAM_BOT_TOKEN"
default_agent = "coder"
...
Copy any `MEMORY.md` files from OpenClaw agents to Tapthe.ai agent directories:

```bash
cp ~/.openclaw/agents/coder/MEMORY.md ~/.tapthe-ai/agents/coder/imported_memory.md
```
...
| Aspect | OpenClaw | Tapthe.ai |
|--------|----------|----------|
| Format | YAML | TOML |
| Config location | `~/.openclaw/config.yaml` | `~/.tapthe-ai/config.toml` |
...
Tools were renamed between OpenClaw and Tapthe.ai for consistency. The migration engine handles this automatically.

| OpenClaw Tool | Tapthe.ai Tool | Notes |
|---------------|---------------|-------|
...
| OpenClaw Profile | Tapthe.ai Tools |
|------------------|----------------|
...
| OpenClaw Name | Tapthe.ai Name | API Key Env Var |
|---------------|---------------|-----------------|
...
| **Networking** | None | OFP (Tapthe.ai Protocol) peer-to-peer |
...
```bash
tapthe-ai migrate --from openclaw --source-dir /path/to/your/workspace
```
...
```bash
tapthe-ai skill install /path/to/openclaw/skills/my-skill
```
...
```bash
cat ~/.tapthe-ai/channels_import.toml
# Copy the [channels.*] sections into ~/.tapthe-ai/config.toml
```
