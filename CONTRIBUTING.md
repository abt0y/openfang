# Contributing to Tapthe.ai

Thank you for your interest in contributing to Tapthe.ai. This guide covers everything you need to get started, from setting up your development environment to submitting pull requests.
...
```bash
git clone https://github.com/RightNow-AI/tapthe-ai.git
cd tapthe-ai
cargo build
```
...
```bash
cargo build --profile release-fast -p tapthe-ai-cli
```
...
```bash
cargo test -p tapthe-ai-kernel
cargo test -p tapthe-ai-runtime
cargo test -p tapthe-ai-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `TaptheAiKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `tapthe-ai-{name}` (kebab-case)
...
Tapthe.ai is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `tapthe-ai-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `tapthe-ai-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
| `tapthe-ai-runtime` | Agent loop, 3 LLM drivers (Anthropic/Gemini/OpenAI-compat), 38 built-in tools, WASM sandbox, MCP client/server, A2A protocol |
| `tapthe-ai-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `tapthe-ai-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `tapthe-ai-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `tapthe-ai-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `tapthe-ai-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `tapthe-ai-wire` | OFP (Tapthe.ai Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `tapthe-ai-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `tapthe-ai-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `tapthe-ai-skills` | Skill system: 60 bundled skills, FangHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `tapthe-ai-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `tapthe-ai-runtime`, implemented on `TaptheAiKernel` in `tapthe-ai-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.tapthe-ai/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
```bash
tapthe-ai agent spawn agents/my-agent/agent.toml
```
...
Channel adapters live in `crates/tapthe-ai-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/tapthe-ai-channels/src/myplatform.rs`
...
3. Register the module in `crates/tapthe-ai-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/tapthe-ai-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `tapthe-ai-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/tapthe-ai-cli/src/main.rs` under `cmd_channel_setup`.
...
Built-in tools are defined in `crates/tapthe-ai-runtime/src/tool_runner.rs`.
...
- Open a [GitHub Discussion](https://github.com/RightNow-AI/tapthe-ai/discussions) for questions.
- Open a [GitHub Issue](https://github.com/RightNow-AI/tapthe-ai/issues) for bugs or feature requests.
