# Tapthe.ai — Agent Instructions

## Project Overview
Tapthe.ai is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.tapthe-ai/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/tapthe-ai.exe` (or `target/debug/tapthe-ai.exe`)
...
```bash
GROQ_API_KEY=<key> target/release/tapthe-ai.exe start &
...
## Architecture Notes
- **Don't touch `tapthe-ai-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `tapthe-ai.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
