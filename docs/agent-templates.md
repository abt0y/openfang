...
Tapthe.ai ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
tapthe-ai spawn orchestrator
tapthe-ai spawn coder
tapthe-ai spawn --template agents/writer/agent.toml
```
...
```bash
tapthe-ai spawn security-auditor
...
```bash
tapthe-ai spawn data-scientist
...
```bash
tapthe-ai spawn legal-assistant
...
> General-purpose assistant. The default Tapthe.ai agent for everyday tasks, questions, and conversations.
...
```bash
tapthe-ai spawn email-assistant
...
```bash
tapthe-ai spawn customer-support
...
```bash
tapthe-ai spawn sales-assistant
...
```bash
tapthe-ai spawn meeting-assistant
...
```bash
tapthe-ai spawn health-tracker
...
```bash
tapthe-ai spawn personal-finance
...
```bash
tapthe-ai spawn travel-planner
...
```bash
tapthe-ai spawn home-automation
...
tapthe-ai spawn coder --name "backend-coder"

# Spawn from a TOML file path
tapthe-ai spawn --template agents/custom/my-agent.toml

# List running agents
tapthe-ai agents

# Send a message
tapthe-ai message <agent-id> "Write a function to parse TOML files"

# Kill an agent
tapthe-ai kill <agent-id>
```
