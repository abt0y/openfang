# Tapthe.ai Configuration Reference

Complete reference for `config.toml`, covering every configurable field in the Tapthe.ai Agent OS.
...
Tapthe.ai reads its configuration from a single TOML file:
...
The simplest working configuration only needs an LLM provider API key set as an environment variable. With no config file at all, Tapthe.ai boots with Anthropic as the default provider:
...
```toml
# ============================================================
# Tapthe.ai Agent OS -- Complete Configuration Reference
# ============================================================

# --- Top-level fields ---
home_dir = "~/.tapthe-ai"             # Tapthe.ai home directory
data_dir = "~/.tapthe-ai/data"        # SQLite databases and data files
...
# --- Memory ---
[memory]
# sqlite_path = "~/.tapthe-ai/data/tapthe-ai.db"  # Auto-resolved if omitted
...
|-------|------|---------|-------------|
| `home_dir` | path | `~/.tapthe-ai` | Tapthe.ai home directory. Stores config, agents, skills. |
| `data_dir` | path | `~/.tapthe-ai/data` | Directory for SQLite databases and persistent data. |
...
```toml
[memory]
# sqlite_path = "/custom/path/tapthe-ai.db"
embedding_model = "all-MiniLM-L6-v2"
consolidation_threshold = 10000
decay_rate = 0.1
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `sqlite_path` | path or null | `null` | Explicit path to the SQLite database file. When `null`, defaults to `{data_dir}/tapthe-ai.db`. |
...
Configures the OFP (Tapthe.ai Protocol) peer-to-peer networking layer with HMAC-SHA256 mutual authentication.
...
| `user_id` | string | `""` | Bot user ID (e.g., `"@tapthe-ai:matrix.org"`). |
...
| `nick` | string | `"tapthe-ai"` | Bot nickname. |
| `password_env` | string or null | `null` | Env var holding the server password (optional). |
| `channels` | list of strings | `[]` | IRC channels to join (e.g., `["#tapthe-ai", "#general"]`). |
...
| `nick` | string | `"tapthe-ai"` | Bot nickname in Twitch chat. |
| `default_agent` | string or null | `null` | Agent name to route messages to. |
...
| `username` | string | `"tapthe-ai"` | Bot username in Mumble. |
...
Agent-to-Agent protocol configuration, enabling inter-agent communication across Tapthe.ai instances.
