...
cargo tauri signer generate -w ~/.tauri/tapthe-ai.key
```
...
Your private key was saved to: ~/.tauri/tapthe-ai.key
```
...
Open `crates/tapthe-ai-desktop/tauri.conf.json` and replace:
...
|---|---|---|
| `TAURI_SIGNING_PRIVATE_KEY` | Contents of `~/.tauri/tapthe-ai.key` | Yes |
...
The following icon files must exist in `crates/tapthe-ai-desktop/icons/`:
...
## 5. Set Up the `tapthe-ai.sh` Domain

**Status:** BLOCKING for install scripts — users run `curl -sSf https://tapthe-ai.sh | sh`.

Options:
- **GitHub Pages**: Point `tapthe-ai.sh` to a GitHub Pages site that redirects `/` to `scripts/install.sh` and `/install.ps1` to `scripts/install.ps1` from the repo's latest release.
- **Cloudflare Workers / Vercel**: Serve the install scripts with proper `Content-Type: text/plain` headers.
- **Raw GitHub redirect**: Use `tapthe-ai.sh` as a CNAME to `raw.githubusercontent.com/RightNow-AI/tapthe-ai/main/scripts/install.sh` (less reliable).

The install scripts reference:
- `https://tapthe-ai.sh` → serves `scripts/install.sh`
- `https://tapthe-ai.sh/install.ps1` → serves `scripts/install.ps1`

Until the domain is set up, users can install via:
```bash
curl -sSf https://raw.githubusercontent.com/RightNow-AI/tapthe-ai/main/scripts/install.sh | sh
```
...
```bash
docker build -t tapthe-ai:local .
docker run --rm tapthe-ai:local --version
docker run --rm -p 4200:4200 -v tapthe-ai-data:/data tapthe-ai:local start
```
...
grep '"version"' crates/tapthe-ai-desktop/tauri.conf.json
...
Visit: `https://github.com/RightNow-AI/tapthe-ai/releases/latest/download/latest.json`
...
docker pull ghcr.io/RightNow-AI/tapthe-ai:0.1.0

# Verify both architectures
docker run --rm ghcr.io/RightNow-AI/tapthe-ai:latest --version
```
...
4. Open the v0.1.0 app — after 10 seconds it should:
   - Show "Tapthe.ai Updating..." notification
...
tapthe-ai --version  # Should print v0.1.0
