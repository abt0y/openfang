...
Skills are pluggable tool bundles that extend agent capabilities in Tapthe.ai. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
Skills are installed to `~/.tapthe-ai/skills/` and made available to agents through the skill registry. Tapthe.ai ships with **60 bundled skills** that are compiled into the binary and available immediately.
...
Tapthe.ai includes 60 expert knowledge skills compiled into the binary (no installation needed):
...
1. Tapthe.ai sends a JSON payload to the script's stdin:
...
```python
#!/usr/bin/env python3
"""Tapthe.ai skill: web-summarizer"""
import json
import sys
...
def summarize_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Tapthe.ai-Skill/1.0"})
...
### Using the Tapthe.ai Python SDK

For more advanced skills, use the Python SDK (`sdk/python/tapthe_ai_sdk.py`):

```python
#!/usr/bin/env python3
from tapthe_ai_sdk import SkillHandler
...
```bash
tapthe-ai skill install /path/to/my-skill
```

This reads the `skill.toml`, validates the manifest, and copies the skill to `~/.tapthe-ai/skills/my-skill/`.

### From FangHub

```bash
tapthe-ai skill install web-summarizer
```
...
```bash
tapthe-ai skill install https://github.com/user/tapthe-ai-skill-example.git
```
...
```bash
tapthe-ai skill remove web-summarizer
```
...
FangHub is the community skill marketplace for Tapthe.ai.
...
```bash
tapthe-ai skill search "web scraping"
```
...
Tapthe.ai can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.

### Automatic Conversion

```bash
tapthe-ai skill install /path/to/openclaw-skill
```

If the directory contains an OpenClaw-style skill (Node.js package), Tapthe.ai:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to Tapthe.ai conventions.
4. Copies the skill to the Tapthe.ai skills directory.
...
```bash
tapthe-ai skill install /path/to/skill-directory
```

Skills imported via `tapthe-ai migrate --from openclaw` are also scanned and reported in the migration report, with instructions for manual reinstallation.
