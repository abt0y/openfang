...
Common issues, diagnostics, and answers to frequently asked questions about Tapthe.ai.
...
Tapthe.ai uses `tracing` for structured logging. Set the log level via environment:

```bash
RUST_LOG=info tapthe-ai start          # Default
RUST_LOG=debug tapthe-ai start         # Verbose
RUST_LOG=tapthe-ai=debug tapthe-ai start  # Only Tapthe.ai debug, deps at info
```
...
### `tapthe-ai` command not found after install
...
**Common causes**:
- No API key provided: `docker run -e GROQ_API_KEY=... ghcr.io/RightNow-AI/tapthe-ai`
...
**Fix**: Run `tapthe-ai init` to create the default config:
```bash
tapthe-ai init
```

This creates `~/.tapthe-ai/config.toml` with sensible defaults.
...
```bash
RUST_LOG=tapthe_ai_channels=debug tapthe-ai start
```
...
**Automatic protection**: Tapthe.ai has a built-in loop guard:
...
**Check**:
1. TOML manifest is valid: `tapthe-ai agent spawn --dry-run manifest.toml`
...
2. Set the model to `tapthe-ai:agent-name` (e.g., `tapthe-ai:coder`)
...
3. Try deleting `~/.tapthe-ai/daemon.json` and restarting
...
If slower:
- Check database size (`~/.tapthe-ai/data/tapthe-ai.db`)
...
1. Add the channel config to `~/.tapthe-ai/config.toml` under `[channels]`
2. Set the required environment variables (tokens, secrets)
3. Restart the daemon

### How do I update Tapthe.ai?

```bash
# From source
cd tapthe-ai && git pull && cargo install --path crates/tapthe-ai-cli
...
Only LLM API calls go to the provider's servers. All agent data, memory, sessions, and configuration are stored locally in SQLite (`~/.tapthe-ai/data/tapthe-ai.db`). The OFP wire protocol uses HMAC-SHA256 mutual authentication for P2P communication.

### How do I back up my data?

Back up these files:
- `~/.tapthe-ai/config.toml` (configuration)
- `~/.tapthe-ai/data/tapthe-ai.db` (all agent data, memory, sessions)
- `~/.tapthe-ai/skills/` (installed skills)

### How do I reset everything?

```bash
rm -rf ~/.tapthe-ai
tapthe-ai init  # Start fresh
```

### Can I run Tapthe.ai without an internet connection?
...
### What's the difference between Tapthe.ai and OpenClaw?

| Aspect | Tapthe.ai | OpenClaw |
|--------|----------|----------|
...
Tapthe.ai can import OpenClaw configs: `tapthe-ai migrate --from openclaw`
...
```bash
RUST_LOG=tapthe_ai_runtime=debug,tapthe_ai_channels=info tapthe-ai start
```

### Can I use Tapthe.ai as a library?

Yes. Each crate is independently usable:
```toml
[dependencies]
tapthe-ai-runtime = { path = "crates/tapthe-ai-runtime" }
tapthe-ai-memory = { path = "crates/tapthe-ai-memory" }
```

The `tapthe-ai-kernel` crate assembles everything, but you can use individual crates for custom integrations.
...
### How do I run Tapthe.ai in Docker?

```bash
docker run -d --name tapthe-ai \
...
Tapthe.ai doesn't have built-in login. Use a reverse proxy with basic auth:
...
Not yet — each channel type currently supports one bot. Multi-bot routing is tracked as a feature request (#586). As a workaround, run multiple Tapthe.ai instances on different ports with different configs.

### Claude Code integration shows errors

Add to `~/.tapthe-ai/config.toml`:
```toml
[claude_code]
