# Getting Started with Tapthe.ai

This guide walks you through installing Tapthe.ai, configuring your first LLM provider, spawning an agent, and chatting with it.
...
Download the installer for your platform from the [latest release](https://github.com/RightNow-AI/tapthe-ai/releases/latest):
...
The desktop app includes the full Tapthe.ai system with a native window, system tray, auto-updates, and OS notifications. Updates are installed automatically in the background.

### Option 2: Shell Installer (Linux / macOS)

```bash
curl -sSf https://tapthe-ai.sh | sh
```

This downloads the latest CLI binary and installs it to `~/.tapthe-ai/bin/`.
...
```bash
cargo install --git https://github.com/RightNow-AI/tapthe-ai tapthe-ai-cli
```
...
```bash
docker pull ghcr.io/RightNow-AI/tapthe-ai:latest

docker run -d \
  --name tapthe-ai \
  -p 4200:4200 \
  -e ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY \
  -v tapthe-ai-data:/data \
  ghcr.io/RightNow-AI/tapthe-ai:latest
```
...
Run the init command to create the `~/.tapthe-ai/` directory and a default config file:
...
Tapthe.ai needs at least one LLM provider API key. Set it as an environment variable:
...
The default config uses Anthropic. To change the provider, edit `~/.tapthe-ai/config.toml`:
...
Tapthe.ai ships with 30 agent templates. Spawn the hello-world agent:

```bash
tapthe-ai agent spawn agents/hello-world/agent.toml
```
...
```bash
tapthe-ai agent spawn my-agent.toml
```
...
```bash
tapthe-ai agent chat a1b2c3d4-e5f6-...
```
...
agent> I'm the hello-world agent running on Tapthe.ai. I can:
...
```
Starting Tapthe.ai daemon...
Tapthe.ai daemon running on http://127.0.0.1:4200
...
Now that you have Tapthe.ai running:
...
- **Set up workflows**: Chain multiple agents together. Use `tapthe-ai workflow create` with a TOML workflow definition.
- **Use MCP**: Connect to external tools via Model Context Protocol. Configure in `config.toml` under `[[mcp_servers]]`.
- **Migrate from OpenClaw**: Run `tapthe-ai migrate --from openclaw`. See [MIGRATION.md](../MIGRATION.md).
- **Desktop app**: Run `cargo tauri dev` for a native desktop experience with system tray.
- **Run diagnostics**: `tapthe-ai doctor` checks your entire setup.

### Useful Commands Reference

```bash
tapthe-ai init                          # Initialize ~/.tapthe-ai/
tapthe-ai start                         # Start the daemon
tapthe-ai status                        # Check daemon status
tapthe-ai doctor                        # Run diagnostic checks

tapthe-ai agent spawn <manifest.toml>   # Spawn an agent
tapthe-ai agent list                    # List all agents
tapthe-ai agent chat <id>               # Chat with an agent
tapthe-ai agent kill <id>               # Kill an agent

tapthe-ai workflow list                 # List workflows
tapthe-ai workflow create <file.json>   # Create a workflow
tapthe-ai workflow run <id> <input>     # Run a workflow

tapthe-ai trigger list                  # List event triggers
tapthe-ai trigger create <args>         # Create a trigger
tapthe-ai trigger delete <id>           # Delete a trigger

tapthe-ai skill install <source>        # Install a skill
tapthe-ai skill list                    # List installed skills
tapthe-ai skill search <query>          # Search FangHub
tapthe-ai skill create                  # Scaffold a new skill

tapthe-ai channel list                  # List channel status
tapthe-ai channel setup <channel>       # Interactive setup wizard

tapthe-ai config show                   # Show current config
tapthe-ai config edit                   # Open config in editor

tapthe-ai chat [agent]                  # Quick chat (alias)
tapthe-ai migrate --from openclaw       # Migrate from OpenClaw
tapthe-ai mcp                           # Start MCP server (stdio)
```
