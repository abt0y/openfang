...
Tapthe.ai is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).

```
tapthe-ai-cli            CLI interface, daemon auto-detect, MCP server
    |
tapthe-ai-desktop        Tauri 2.0 desktop app (WebView + system tray)
    |
tapthe-ai-api            REST/WS/SSE API server (Axum 0.8), 76 endpoints
    |
tapthe-ai-kernel         Kernel: assembles all subsystems, workflow engine, RBAC, metering
    |
    +-- tapthe-ai-runtime    Agent loop, 3 LLM drivers, 23 tools, WASM sandbox, MCP, A2A
    +-- tapthe-ai-channels   40 channel adapters, bridge, formatter, rate limiter
    +-- tapthe-ai-wire       OFP peer-to-peer networking with HMAC-SHA256 auth
    +-- tapthe-ai-migrate    Migration engine (OpenClaw YAML->TOML)
    +-- tapthe-ai-skills     60 bundled skills, FangHub marketplace, ClawHub client
    |
tapthe-ai-memory         SQLite memory substrate, sessions, semantic search, usage tracking
    |
tapthe-ai-types          Shared types: Agent, Capability, Event, Memory, Message, Tool, Config,
...
When `TaptheAiKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:

```
1. Load configuration
   - Read ~/.tapthe-ai/config.toml (or specified path)
   - Apply #[serde(default)] defaults for missing fields
   - Validate config and log warnings (missing API keys, etc.)

2. Create data directory
   - Ensure ~/.tapthe-ai/data/ exists

3. Initialize memory substrate
   - Open SQLite database (tapthe-ai.db)
...
The memory substrate (`tapthe-ai-memory`) provides six layers of storage:
...
The `LlmDriver` trait (`tapthe-ai-runtime`) provides a unified interface for all LLM providers:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, TaptheAiError>;
...
The `ModelCatalog` (`tapthe-ai-runtime/src/model_catalog.rs`) provides a registry of all known models, providers, and aliases.
...
Tapthe.ai implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
`taint.rs` in `tapthe-ai-types` implements taint labels and taint sets. Data from external sources carries taint labels that propagate through operations, enabling information flow analysis.
...
The channel system (`tapthe-ai-channels`) provides 40 adapters for messaging platform integration.
...
The skill system (`tapthe-ai-skills`) provides 60 bundled skills and supports external skill installation.
...
- **FangHub**: Native Tapthe.ai marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-Tapthe.ai tool name mappings in `tool_compat.rs`.
...
Tapthe.ai implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Tapthe.ai's 23 built-in tools via the MCP protocol. Enables external tools to use Tapthe.ai as a tool provider.
...
The Tapthe.ai Protocol (OFP) enables peer-to-peer agent communication across machines.
...
The desktop app (`tapthe-ai-desktop`) wraps the full Tapthe.ai stack in a native Tauri 2.0 application.
...
+-------------------------------------------------------------------+
    ...
    v                        v                   v         v
+------------------+  +--------------+  +--------+  +-----------+
| tapthe-ai-runtime |  | tapthe-ai-    |  | open-  |  | tapthe-ai- |
